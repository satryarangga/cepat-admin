    ...
    <div class="box">
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-hover table-striped">
                ...
                <td>{{$val->name}}</td>
                <td>{{$val->desc}}</td>
                <td>{{($val->confirm_type == 1) ? 'Manual' : 'Automatic'}}</td>
                <td>{{($val->minimum_payment) ? moneyFormat($val->minimum_payment) : 'No minimum payment'}}</td>
                <td><img src="{{asset('images/payment-method/'.$val->logo)}}" style="width: 50; height: 25px" /></td>
